...
class BelongsToMany extends Relation
{
     ...
     * @var string
     */
    protected $parentKeyMethod;
    /**
     * @var string
     */
    protected $foreignPivotKeyMethod;
    /**
     * The name of the accessor to use for the "pivot" relationship.
     *
     * @var string
     */
    protected $accessor = 'pivotAttr';
    /**
     ...
     * @return void
     */
    public function __construct(Builder $query, Model $parent, Model $pointEntity, $foreignPivotKey,
                                $relatedPivotKey, $parentKey, $relatedKey, $relationName = null)
    {
        ...
        $this->parentKeyMethod = sprintf('get%s', ucfirst(Str::camel($parentKey)));
        $this->foreignPivotKeyMethod = sprintf('get%s', ucfirst(Str::camel($foreignPivotKey)));
     ...
     *
     * @param \Swoft\Db\Eloquent\Builder|null $query
     * @return $this
     */
    protected function performJoin($query = null)
    {
        ...
        $pointTable = $this->pointEntity->getTable();

        $key = $baseTable . '.' . $this->relatedKey;

        $query->join($pointTable, $key, '=', $this->getQualifiedRelatedPivotKeyName());
    ...
    {
        $this->query->where(
            $this->getQualifiedForeignPivotKeyName(), '=', $this->parent->{$this->parentKeyMethod}()
        );
    ...
    public function initRelation(array $models, $relation)
    {
        foreach ($models as $model) {
            $this->related->setPointAttr($this->pointEntity);
            $relationModel = $this->related->newCollection();
            $model->setRelation($relation, $relationModel);
        }
    ...
    public function match(array $models, Collection $results, $relation)
    {
        ...
        /** @var Model $model */
        foreach ($models as $model) {
            if (isset($dictionary[$key = $model->{$this->parentKeyMethod}()])) {
    ...
    protected function buildDictionary(Collection $results)
    {
        ...
        foreach ($results as $result) {
            if(!(($model = $result[$this->accessor]) instanceof Model)) {
                continue;
            }
            $dictionary[$model->{$this->foreignPivotKeyMethod}()][] = $result;
        }
    ...
    public function wherePivot($column, $operator = null, $value = null, $boolean = 'and')
    {
        ...
        return $this->where($this->pointEntity->getTable() . '.' . $column, $operator, $value, $boolean);
    }
    ...
    public function wherePivotIn($column, $values, $boolean = 'and', $not = false)
    {
        ...
        return $this->whereIn($this->pointEntity->getTable() . '.' . $column, $values, $boolean, $not);
    }
     ...
     * @param mixed $id
     * @param array $columns
     * @return \Illuminate\Support\Collection|\Swoft\Db\Eloquent\Model
     */
     ...
     * @param mixed $id
     * @param array $columns
     * @return \Swoft\Db\Eloquent\Model|\Swoft\Stdlib\Collection|null
     */
    ...
    {
        return !is_null($this->parent->{$this->parentKeyMethod}())
            ? $this->get()
    ...
    public function get($columns = ['*'])
    {
        ...
        $models = $builder->getQuery()->addSelect(
            $this->shouldSelect($columns)
        )->get();
        $models = $this->hydratePivotRelation($models);
     ...
     * @return void
     */
    protected function hydratePivotRelation(Collection $models)
    {
        $models = $models->map(function ($item) {
            $item[$this->accessor] = $this->newExistingPivot(
                $this->migratePivotAttributes($item)
            );
            return $item;
        });
        return $models;
     ...
     *
     * @param \Swoft\Db\Eloquent\Model $model
     * @return array
     */
    ...
    {
        return $this->pointEntity->getTable() . '.' . $this->foreignPivotKey;
    }
    ...
    {
        return $this->pointEntity->getTable() . '.' . $this->relatedPivotKey;
    }
    ...
    {
        return $this->pointEntity->getTable();
    }
